# CLAUDE.md — GrowGrade: Аудит кодовой базы
...
GrowGrade — образовательная платформа для IT-специалистов с внутренней валютой CodeCoins, каталогом курсов, чатом, сообществами, RAG-поиском по PDF и AI-ассистентом. Проект состоит из **React 19 SPA-фронтенда** и **микросервисного бэкенда** (7 сервисов на FastAPI + nginx-шлюз + Kafka + PostgreSQL + Redis + ChromaDB).
